...
verify "lynx"
# andrewt verify "tiff2ps"
# andrewt verify "ps2pdf"
verify "sed"
verify "head"
verify "mv"
# andrewt verify "rm"
...
PAGE=1
{
  while [ "$PAGE" -le "$NUMPAGES" ] ; do
	  file="pat${PATNUM}-${PAGE}.tiff"
    werr "...fetching page $PAGE of $NUMPAGES to file: $file"
    url2stdout "${TIFFURL1}${PAGE}${TIFFURL2}" > $file || die "Error retrieving TIFF page."
    # andrewt tiff2ps "tmppat${PATNUM}.tiff" 2>/dev/null || die "tiff2ps error"
    PAGE=$[$PAGE+1]
  done
}
...
# andrewt rm -f "tmppat${PATNUM}.tiff"

# kludge around differences in versions of pat2pdf
# andrewt mv -f "tmppat${PATNUM}.pdf.pdf" "tmppat${PATNUM}.pdf" &>/dev/null
# andrewt mv -f "tmppat${PATNUM}.pdf" "pat${PATNUM}.pdf" || die "Error renaming pdf file."

werr Done.

# andrewt werr pat${PATNUM}.pdf
